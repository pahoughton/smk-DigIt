...
@implementation GradyView
@synthesize myGradient;
@synthesize myStartColor;
@synthesize myEndColor;
@synthesize forceColorChange;
@synthesize myAngle;
@synthesize myIsRadial;
@synthesize myOffsetPt;
...
- (NSPoint)getRelativeCenterPositionFromEvent:(NSEvent*)theEvent
{
  ...
  NSPoint pt = NSMakePoint( (curMousePt.x - NSMidX([self bounds])) 
                           / ([self bounds].size.width / 2.0),
                           (curMousePt.y - NSMidY([self bounds])) 
...
- (void)resetGradient
{
  if( forceColorChange || myGradient == nil ) {
    myGradient = [[NSGradient alloc] initWithStartingColor:myStartColor 
                                               endingColor:myEndColor];
    forceColorChange = NO;    
  }
}
